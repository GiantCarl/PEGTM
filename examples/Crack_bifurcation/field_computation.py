...
class FieldComputation:
    ...
    def fieldCalculation(self, inp,world_pos,hist_alpha ):
        ...
        u = ((inp.node_features[:, 1]-y0)*(yL-inp.node_features[:, 1])*out_disp[:, 0] + \
             (inp.node_features[:, 1]-y0)/(yL-y0)*torch.cos(self.theta))*self.lmbda_delta + (world_pos[:,0] - inp.node_features[:,0]) * self.non_dim 
        v = ((inp.node_features[:, 1]-y0)*(yL-inp.node_features[:, 1])*out_disp[:, 1] + \
             (inp.node_features[:, 1]-y0)/(yL-y0)*torch.sin(self.theta))*self.lmbda_delta + (world_pos[:,1] - inp.node_features[:,1]) * self.non_dim 

...
class FieldComputation:
    ...
    def fieldCalculation(self, inp, world_pos,hist_alpha):
        ...
        fix_window = torch.ones_like(inp.node_features[:,0])
        condition = (inp.node_features[:,1] == self.y0)
        rows_indices = torch.nonzero(condition).squeeze()
        fix_window[rows_indices] = 0

        load_window = torch.ones_like(inp.node_features[:,0])
        condition = (inp.node_features[:,0] >= 0.44) & (inp.node_features[:,0]  <= 0.5) & (inp.node_features[:,1] == 0)
        rows_indices = torch.nonzero(condition).squeeze()
        load_window[rows_indices] = 0

        u = fix_window *out_disp[:, 0] * self.lmbda_delta + (world_pos[:,0] - inp.node_features[:,0]) * self.non_dim 
        v = (fix_window * load_window *out_disp[:, 1] + (1-load_window)) * self.lmbda_delta + (world_pos[:,1] - inp.node_features[:,1]) * self.non_dim 

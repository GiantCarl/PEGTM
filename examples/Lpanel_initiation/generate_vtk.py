...
disp_x_node = graph_data["fix_X"]
disp_y_node = graph_data["fix_Y"]
...
for i in range(len(npz_file)):
    ...
    energy = np.append(energy, np.array([[disp[i]*np.sin(field_comp.theta.numpy().squeeze()) * transform_disp, E_el * transform_energy, E_d * transform_energy]]), axis = 0)
    # 将网格写入.vtu文件
    ...
    force_x = torch.sum(torch.mean(stress_12_node[edge_x],dim=-1) * relative_edge_mesh_pos[matching_rows_x]).numpy() 
    force_y = torch.sum(torch.mean(stress_22_node[edge_y],dim=-1) * relative_edge_mesh_pos[matching_rows_y]).numpy() 

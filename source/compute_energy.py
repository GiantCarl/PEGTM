...
def compute_energy(inp, u, v, alpha, hist_alpha, matprop, pffmodel):
    E_el, E_d = compute_energy_per_elem(inp.node_features[:,0:2], u, v, alpha, 
                                                        hist_alpha, matprop, pffmodel, inp.area_elem, inp.T_conn)
    E_el_sum = torch.sum(E_el)
    E_d_sum = torch.sum(E_d)

    return E_el_sum, E_d_sum
